...
public enum GameMenuCommand implements Command {
    ...
    SHOW_CRAFT_INFO("^\\s*craft\\s+Info\\s+-n\\s+(?<craft-name>[A-Za-z ]+)\\s*$"),
    ...
    GIFT_NPC("^\\s*Gift\\s+NPC\\s+(?<npcName>\\S+)\\s+-i\\s+(?<item>[A-Za-z ]+)\\s+-a\\s+(?<amount>\\d+)\\s*$"),
    LIST_NPC_FRIENDSHIPS("^\\s*friendship\\s+NPC\\s+list\\s*$"),
    LIST_QUESTS("^\\s*quests\\s+list\\s+(?<npcName>\\S+)\\s*$"),
    TREE_INFO("^\\s*tree\\s+info\\s+(?<name>[A-Za-z ]+)\\s*$"),
    CROP_INFO("^\\s*crop\\s+info\\s+(?<name>[A-Za-z ]+)\\s*$"),
    FORAGING_CROP_INFO("^\\s*foraging\\s+crop\\s+info\\s+(?<name>[A-Za-z ]+)\\s*$"),
    FORAGING_TREE_INFO("^\\s*foraging\\s+tree\\s+info\\s+(?<name>[A-Za-z ]+)\\s*$"),
    FRUITS_INFO("^\\s*fruit\\s+info\\s+(?<name>[A-Za-z ]+)\\s*$"),
    COMPLETE_QUEST("^\\s*quests\\s+finish\\s+(?<npcName>\\S+)\\s+-i\\s+(?<index>\\d+)\\s*$"),
