            ...
            public void clicked(InputEvent event, float x, float y) {
                if (username.getText().isEmpty() || username.getText().equals("Username")) {
                    ...
                    if (error != null) {
                        ...
                        GameApp.player.setUserInfo(message.getFromBody("player", UserInfo.class));
                        showErrorDialog("Login\nSuccessfully", "Success", Color.GREEN);
                        HashMap<String, Object> body1 = new HashMap<>();
                        body1.put("player", GameApp.player.getUserInfo());
                        Message message1 = GameApp.c2sConnectionThread.sendAndWaitForResponse(new Message(body1, Message.Type.Which_Lobby));
                        if (message1 != null) {
                            GameApp.player.setLobby(message1.getFromBody("lobby", Lobby.class));
                        }
