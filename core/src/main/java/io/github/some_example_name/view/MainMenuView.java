...
public class MainMenuView implements Screen {
    ...
    private final TextButton myLobby;
    ...
    public MainMenuView(Skin skin) {
        ...
        myLobby = new TextButton("My Lobby", skin);
    ...
    public void show() {
        ...
        table.row();
        table.add(myLobby).width(280).height(120).pad(20);
        table.row();
        ...
        myLobby.addListener(new ClickListener() {
            @Override
            public void clicked(InputEvent event, float x, float y) {
                if (GameApp.player.getLobby() == null) {
                    showErrorDialog("First join a lobby\nto show 'My Lobby Menu'", "Warning", Color.YELLOW);
                } else {
                    Main.getMain().setScreen(new MyLobbyMenuView(GameAssetManager.getGameAssetManager().getSkin()));
                }
            }
        });
