...
public enum GameMenuCommand implements Command {
    ...
    TALK_WITH_FRIENDS("^\\s*talk\\s+-u\\s+(?<username>\\S+)\\s+-m\\s+(?<message>\\S+)\\s*$"),
    ...
    SELECT_TRADE("^\\s*trade\\s+-u\\s+(?<username>\\S+)\\s+-t\\s+(?<type>request|offer)(?:\\s+-i\\s+(?<item>\\S+)\\s+-a\\s+(?<amount>\\d+))?(?:\\s+-p\\s+(?<price>\\d+\\.?\\d*))?(?:\\s+-ti\\s+(?<targetItem>\\S+)\\s+-ta\\s+(?<targetAmount>\\d+))?\\s*$"),
