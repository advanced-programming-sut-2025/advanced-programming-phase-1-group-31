...
public enum LoginMenuCommands {
    ENTER_MENU("^menu\\s+enter\\s+(?<menu>\\S+)\\s*$"),
    EXIT_MENU("^menu\\s+exit\\s*$"),
    SHOW_CURRENT_MENU("^show\\s+current\\s+menu\\s*$"),
    REGISTER("^register\\s+-u\\s+(?<username>\\S+)\\s+-p\\s+(?<password>\\S+)\\s+(?<passwordConfirmation>\\S+)\\s+-n\\s+(?<nickname>\\S+)\\s+-e\\s+(?<email>\\S+)\\s+-g\\s+(?<gender>\\S+)\\s*$"),
